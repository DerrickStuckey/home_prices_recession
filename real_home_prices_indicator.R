...
# View(real.res.prices.schiller)

### look only at data after 1976
real.res.prices.recent <- real.res.prices.schiller[real.res.prices.schiller$Date.Formatted>=as.Date("1976-01-01"),]
...
# ggsave(filename = "./charts/rhp_recessions_ind_1976_pres_nr.png")
...
# ggsave(filename = "./charts/rhp_recessions_ind_1976_pres.png")


### Try for period 1953 - 1976 (1953 is the earliest year with monthly home price data)
real.res.prices.midcent <- real.res.prices.schiller[
  real.res.prices.schiller$Date.Formatted>=as.Date("1953-01-01") &
  real.res.prices.schiller$Date.Formatted<as.Date("1976-01-01"),]
...
# Real home prices 1953 - 1976 with recessions shaded
# and declines of 3% or more from the previous peak flagged
ggplot(data=real.res.prices.midcent) + 
  geom_line(aes(x=Date.Formatted,y=Real.Home.Price.Index),color="blue") + 
  ggtitle("Real Home Prices 1953-1976\nDeclines of 3% in Red") + 
  xlab("Date") + ylab("Real Home Prices") + 
  theme_light() + theme(plot.title = element_text(hjust = 0.5)) + 
  geom_hline(yintercept=0) + 
...
# ggsave(filename = "./charts/rhp_recessions_ind_1953_1976.png")
...
ggplot(data=real.res.prices.old) + 
  geom_line(aes(x=Date.Formatted,y=Real.Home.Price.Index),color="blue") + 
  ggtitle("Real Home Prices 1890-1953\nDeclines of 3% in Red") + 
  xlab("Date") + ylab("Real Home Prices") + 
  theme_light() + theme(plot.title = element_text(hjust = 0.5)) + 
  geom_hline(yintercept=0) + 
